...
import type { Blog, BlogList, BlogSortField } from './types';
import type { BlogDetailOptions, BlogsSearchParams } from './schemas';

// @export
export type BlogSummary = Omit<Blog, 'content'>;

// @export
export type BlogDetail = Omit<Blog, 'preview' | 'content'> & Pick<Blog, 'preview' | 'content'>;
...
export function getBlogBySlug(
  slug: string,
  options: BlogDetailOptions = { preview: false, content: true },
): BlogDetail | undefined {
  const blog = blogs.find(({ slugName }) => slugName === slug);

  if (!blog) return undefined;

  const { preview, content, ...detail } = blog;
  return {
    ...detail,
    ...(options.preview && preview !== undefined ? { preview } : {}),
    ...(options.content && content !== undefined ? { content } : {}),
  };
}
